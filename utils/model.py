...
class CustomDataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = scipy.io.loadmat(img_name)
        keys = [key for key in img.keys() if not key.startswith('__')]
        img = img[keys[0]]
        # img = img['slice_data']
        tensor_image = torch.from_numpy(img).unsqueeze(0).float()
        ...
        for i, stride in enumerate(kargs['stride_list']):
            ...
            if kargs['sin_res']:
                if i == len(kargs['stride_list']) - 1:
                    head_layer = nn.Conv2d(ngf, 1, 1, 1, bias=kargs['bias']) 
                else:
                    head_layer = None
            else:
                head_layer = nn.Conv2d(ngf, 1, 1, 1, bias=kargs['bias'])
