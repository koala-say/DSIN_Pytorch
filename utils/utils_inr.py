...
def loss_fn(pred, target, args, prev_output=None, prev_target = None):
    target = target.detach()
    prev_output = prev_output.detach()
    prev_target = prev_target.detach()

    if args.loss_type == 'L2+TV':
        loss = F.mse_loss(pred, target, reduction='none')
        loss = 0.9 * loss.mean() + 0.1 * tv_loss(pred - prev_output)
    elif args.loss_type == 'Ltv':
        loss = F.mse_loss(pred, target, reduction='none') + 0.2 * F.mse_loss(pred - prev_output, target - prev_target , reduction='none')
        loss = loss.mean()
    elif args.loss_type == 'smooth_l1_loss':
        loss =  0.8 * F.mse_loss(pred, target, reduction='none') +0.2 * F.smooth_l1_loss(pred - prev_output, target - prev_target , reduction='none')
        loss = loss.mean()
    elif args.loss_type == 'l1_loss':
        loss =  0.8 * F.mse_loss(pred, target, reduction='none') +0.2 * F.l1_loss(pred - prev_output, target - prev_target , reduction='none')
        loss = loss.mean()
